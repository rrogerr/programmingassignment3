...
rankhospital <- function(state, outcome, num = "best") {
        ...
        if(no_NA$A[num] == no_NA$A[num + 1] | no_NA$A[num] == no_NA$A[num - 1]){
                ...
                #return(tie)
                hospp <- tie[order(tie$state_hosp_ord.Hospital.Name),]$state_hosp_ord.Hospital.Name[1]
                return(as.character(hospp))
        }
        
        
        as.character(no_NA$state_hosp_ord.Hospital.Name[num])
}
